...
from pyfva.soap.validador_documento import ValideElServicio as ValideServicioDocumento,\
    ...
    ValideElDocumentoPdf, ValideElDocumentoJsonEnvelopingCoFirma, \
...
from pyfva.constants import get_text_representation, ERRORES_VALIDA_CERTIFICADO,\
    ...
    ERRORES_VALIDAR_PDF, ERRORES_VALIDAR_JSON
import traceback

from pyfva import logger
from pyfva.soap.validador_documento import ValidadorDeDocumentosSoapServiceStub
...
class ClienteValidador(object):
    ...
    * PDF: .pdf
    ...
    def validar_documento(self, documento, formato):
        ...
        :param formato: tipo de documento a validar (cofirma, contrafirma, json_cofirma, json_fhir, msoffice, odf, pdf).
        ...
        **DEFAULT_DOCUMENT_ERROR**.
        ...
        try:
            ...
            elif formato == 'json_cofirma':
                dev = self._validar_documento_json_cofirma(documento)
            elif formato == 'json_fhir':
                dev = self._validar_documento_json_fhir(documento)
    ...
    def _validar_documento_json_cofirma(self, documento):
        stub = ValidadorDeDocumentosSoapServiceStub()
        options = ValideElDocumentoJsonEnvelopingCoFirma()
        options.elDocumento = documento
        try:
            status = stub.ValideElDocumentoJsonEnvelopingCoFirma(options)
            dev = self._extract_documento(
                status.soap_body.ValideElDocumentoJsonEnvelopingCoFirmaResult,
                ERRORES_VALIDAR_JSON)
        except Exception as e:
            logger.error({'message':"Validador: validando  json cofirma", 'data': e, 'location': __file__}, e=e)
            dev = self.DEFAULT_DOCUMENT_ERROR(ERRORES_VALIDAR_JSON)
            dev['extrar_error'] = str(e)
        return dev

    def _validar_documento_json_fhir(self, documento):
        stub = ValidadorDeDocumentosSoapServiceStub()
        options = ValideElDocumentoJsonParaArchivosFHIR()
        options.elDocumento = documento
        try:
            status = stub.ValideElDocumentoJsonParaArchivosFHIR(options)
            dev = self._extract_documento(
                status.soap_body.ValideElDocumentoJsonParaArchivosFHIRResult,
                ERRORES_VALIDAR_JSON)
        except Exception as e:
            logger.error({'message':"Validador: validando  json fhir", 'data': e, 'location': __file__}, e=e)
            dev = self.DEFAULT_DOCUMENT_ERROR(ERRORES_VALIDAR_JSON)
            dev['extrar_error'] = str(e)
        return dev
